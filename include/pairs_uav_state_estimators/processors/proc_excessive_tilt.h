...
private:
  ...
  pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped> sh_orientation_;
...
ProcExcessiveTilt<n_measurements>::ProcExcessiveTilt(const rclcpp::Node::SharedPtr &node, const std::string &correction_name, const std::string &name,
  ...
  sh_orientation_ = pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(shopts, "/" + ch->uav_name + "/" + orientation_topic_);
}
/*//}*/
...
std::tuple<bool, bool> ProcExcessiveTilt<n_measurements>::process([[maybe_unused]] measurement_t &measurement) {
  ...
  try {
    Eigen::Matrix3d orientation_R = pairs_lib::AttitudeConverter(sh_orientation_.getMsg()->quaternion);

    const double tilt = pairs_lib::geometry::angleBetween(Eigen::Vector3d(0, 0, 1), orientation_R.col(2));
