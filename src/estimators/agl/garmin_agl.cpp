...
void GarminAgl::initialize(ros::NodeHandle &nh, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  ph_agl_height_ = pairs_lib::PublisherHandler<pairs_msgs::Float64Stamped>(nh, Support::toSnakeCase(getName()) + "/agl_height", 10);
  if (ch_->debug_topics.covariance) {
    ph_agl_height_cov_ = pairs_lib::PublisherHandler<pairs_msgs::Float64ArrayStamped>(nh, Support::toSnakeCase(getName()) + "/agl_height_cov", 10);
  }
...
void GarminAgl::timerUpdate([[maybe_unused]] const ros::TimerEvent &event) {
  ...
  pairs_msgs::Float64Stamped agl_height = agl_height_init_;
  agl_height.header.stamp             = time_now;
  agl_height.value                    = est_agl_garmin_->getState(POSITION);

  pairs_msgs::Float64ArrayStamped agl_height_cov;
...
/*//{ getUavAglHeight() */
pairs_msgs::Float64Stamped GarminAgl::getUavAglHeight() const {
