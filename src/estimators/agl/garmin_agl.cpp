...
void GarminAgl::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  ph_agl_height_ = pairs_lib::PublisherHandler<pairs_msgs::msg::Float64Stamped>(node_, Support::toSnakeCase("~/" + getName()) + "/agl_height");

  if (ch_->debug_topics.covariance) {
    ph_agl_height_cov_ = pairs_lib::PublisherHandler<pairs_msgs::msg::Float64ArrayStamped>(node_, Support::toSnakeCase("~/" + getName()) + "/agl_height_cov");
  }
...
void GarminAgl::timerUpdate() {
  ...
  pairs_msgs::msg::Float64Stamped agl_height = agl_height_init_;
  agl_height.header.stamp                  = time_now;
  agl_height.value                         = est_agl_garmin_->getState(POSITION);

  pairs_msgs::msg::Float64ArrayStamped agl_height_cov;
  ...
  pairs_lib::set_mutexed(mtx_agl_height_, agl_height, agl_height_);
  pairs_lib::set_mutexed(mtx_agl_height_cov_, agl_height_cov, agl_height_cov_);
...
/*//{ getUavAglHeight() */
pairs_msgs::msg::Float64Stamped GarminAgl::getUavAglHeight() const {
  return pairs_lib::get_mutexed(mtx_agl_height_, agl_height_);
}
/*//}*/

/*//{ getHeightCovariance() */
std::vector<double> GarminAgl::getHeightCovariance() const {
  return pairs_lib::get_mutexed(mtx_agl_height_cov_, agl_height_cov_.values);
}
/*//}*/
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_state_estimators::garmin_agl::GarminAgl, pairs_uav_managers::AglEstimator)
