...
void HdgGeneric::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  error_publisher_ = std::make_unique<pairs_lib::errorgraph::ErrorPublisher>(node_, clock_, "EstimationManager", getPrintName());
  ...
  for (auto corr_name : correction_names_) {
    ...
    corr_ph->param_loader           = std::make_unique<pairs_lib::ParamLoader>(subnode);
  ...
  dynparam_mgr_ = std::make_shared<pairs_lib::DynparamMgr>(node_, mtx_Q_);
  ...
  if (is_repredictor_enabled_) {
    ...
    lkf_rep_              = std::make_unique<pairs_lib::Repredictor<varstep_lkf_t>>(x0, P0, u0, Q_, t0, models_.at(0), rep_buffer_size_);
  ...
  sh_control_input_ = pairs_lib::SubscriberHandler<pairs_msgs::msg::EstimatorInput>(shopts, "~/control_input_in");

  // | ---------------- publishers initialization --------------- |
  if (ch_->debug_topics.input) {
    ph_input_ = pairs_lib::PublisherHandler<pairs_msgs::msg::Float64ArrayStamped>(node_, "~/" + getNamespacedName() + "/input");
  }

  if (ch_->debug_topics.output) {
    ph_output_ = pairs_lib::PublisherHandler<pairs_msgs::msg::EstimatorOutput>(node_, "~/" + getNamespacedName() + "/output");
  }

  if (ch_->debug_topics.diag) {
    ph_diagnostics_ = pairs_lib::PublisherHandler<pairs_msgs::msg::EstimatorDiagnostics>(node_, "~/" + getNamespacedName() + "/diagnostics");
  }
...
void HdgGeneric::timerUpdate() {
  ...
  statecov_t sc = pairs_lib::get_mutexed(mutex_sc_, sc_);
  Q_t        Q  = pairs_lib::get_mutexed(mtx_Q_, Q_);
  try {
  ...
  pairs_lib::set_mutexed(mutex_sc_, sc, sc_);

  if (!isError()) {
    pairs_lib::set_mutexed(mutex_last_valid_hdg_, sc.x(POSITION), last_valid_hdg_);
  }
...
void HdgGeneric::doCorrection(const z_t &z, const double R, const StateId_t &H_idx, const rclcpp::Time &meas_stamp) {
  ...
  if (H_idx == POSITION) {
    ...
    meas(POSITION) = pairs_lib::geometry::radians::unwrap(meas(POSITION), sc.x(POSITION));

    std::scoped_lock lock(mtx_innovation_);

    innovation_(0) = pairs_lib::geometry::radians::dist(pairs_lib::geometry::radians(meas(0)), pairs_lib::geometry::radians(sc.x(POSITION)));
...
double HdgGeneric::getState(const int &state_idx_in) const {
  return pairs_lib::get_mutexed(mutex_sc_, sc_).x(state_idx_in);
}
/*//}*/
...
void HdgGeneric::setState(const double &state_in, const int &state_idx_in) {
  pairs_lib::set_mutexed(mutex_sc_, state_in, sc_.x(state_idx_in));
}
/*//}*/

/*//{ getStates() */
HdgGeneric::states_t HdgGeneric::getStates(void) const {
  return pairs_lib::get_mutexed(mutex_sc_, sc_).x;
}
/*//}*/

/*//{ setStates() */
void HdgGeneric::setStates(const states_t &states_in) {
  pairs_lib::set_mutexed(mutex_sc_, states_in, sc_.x);
}
/*//}*/
...
double HdgGeneric::getCovariance(const int &state_idx_in) const {
  return pairs_lib::get_mutexed(mutex_sc_, sc_).P(state_idx_in, state_idx_in);
}
/*//}*/

/*//{ getCovarianceMatrix() */
HdgGeneric::covariance_t HdgGeneric::getCovarianceMatrix(void) const {
  return pairs_lib::get_mutexed(mutex_sc_, sc_).P;
}
/*//}*/

/*//{ setCovarianceMatrix() */
void HdgGeneric::setCovarianceMatrix(const covariance_t &cov_in) {
  pairs_lib::set_mutexed(mutex_sc_, cov_in, sc_.P);
}
/*//}*/

/*//{ getInnovation() */
double HdgGeneric::getInnovation(const int &state_idx) const {
  return pairs_lib::get_mutexed(mtx_innovation_, innovation_)(state_idx);
}
...
double HdgGeneric::getLastValidHdg() const {
  return pairs_lib::get_mutexed(mutex_last_valid_hdg_, last_valid_hdg_);
}
/*//}*/
