...
void HdgPassthrough::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch,
  ...
  sh_orientation_ = pairs_lib::SubscriberHandler<geometry_msgs::msg::QuaternionStamped>(shopts, "/" + ch_->uav_name + "/" + orient_topic_,
                                                                                      &HdgPassthrough::callbackOrientation, this);

  sh_ang_vel_ = pairs_lib::SubscriberHandler<geometry_msgs::msg::Vector3Stamped>(shopts, "/" + ch_->uav_name + "/" + ang_vel_topic_,
...
void HdgPassthrough::callbackOrientation(const geometry_msgs::msg::QuaternionStamped::ConstSharedPtr msg) {
  ...
  double hdg;
  try {
    hdg = pairs_lib::AttitudeConverter(msg->quaternion).getHeading();
  }
  catch (...) {
  ...
  if (!isError()) {
    pairs_lib::set_mutexed(mutex_last_valid_hdg_, hdg, last_valid_hdg_);
  }
}
/*//}*/
...
void HdgPassthrough::callbackAngularVelocity(const geometry_msgs::msg::Vector3Stamped::ConstSharedPtr msg) {
  ...
  double hdg_rate;
  try {
    hdg_rate = pairs_lib::AttitudeConverter(sh_orientation_.getMsg()->quaternion).getHeadingRate(msg->vector);
  }
  catch (...) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000, "[%s]: failed getting heading", getPrintName().c_str());
  }
...
double HdgPassthrough::getState(const int &state_idx_in) const {
  return pairs_lib::get_mutexed(mtx_hdg_state_, hdg_state_(state_idx_in));
}
/*//}*/
...
void HdgPassthrough::setState(const double &state_in, const int &state_idx_in) {
  ...
  const double prev_hdg_state   = pairs_lib::get_mutexed(mtx_hdg_state_, hdg_state_(state_idx_in));
  prev_hdg_state_(state_idx_in) = prev_hdg_state;
  pairs_lib::set_mutexed(mtx_hdg_state_, state_in, hdg_state_(state_idx_in));

  const double innovation = pairs_lib::geometry::radians::dist(pairs_lib::geometry::radians(state_in), pairs_lib::geometry::radians(prev_hdg_state));
  pairs_lib::set_mutexed(mtx_innovation_, innovation, innovation_(state_idx_in));
}
/*//}*/

/*//{ getStates() */
HdgPassthrough::states_t HdgPassthrough::getStates(void) const {
  return pairs_lib::get_mutexed(mtx_hdg_state_, hdg_state_);
}
/*//}*/

/*//{ setStates() */
void HdgPassthrough::setStates(const states_t &states_in) {
  pairs_lib::set_mutexed(mtx_hdg_state_, states_in, hdg_state_);
}
/*//}*/
...
double HdgPassthrough::getCovariance(const int &state_idx_in) const {
  return pairs_lib::get_mutexed(mtx_hdg_covariance_, hdg_covariance_(state_idx_in, state_idx_in));
}
/*//}*/

/*//{ getCovarianceMatrix() */
HdgPassthrough::covariance_t HdgPassthrough::getCovarianceMatrix(void) const {
  return pairs_lib::get_mutexed(mtx_hdg_covariance_, hdg_covariance_);
}
/*//}*/

/*//{ setCovarianceMatrix() */
void HdgPassthrough::setCovarianceMatrix(const covariance_t &cov_in) {
  pairs_lib::set_mutexed(mtx_hdg_covariance_, cov_in, hdg_covariance_);
}
/*//}*/
