...
void HdgPassthrough::initialize(ros::NodeHandle &nh, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  sh_orientation_ = pairs_lib::SubscribeHandler<geometry_msgs::QuaternionStamped>(shopts, "/" + ch_->uav_name + "/" + orient_topic_,
                                                                                &HdgPassthrough::callbackOrientation, this);
  sh_ang_vel_     = pairs_lib::SubscribeHandler<geometry_msgs::Vector3Stamped>(shopts, "/" + ch_->uav_name + "/" + ang_vel_topic_,
                                                                         &HdgPassthrough::callbackAngularVelocity, this);

  // | ---------------- publishers initialization --------------- |
  if (ch_->debug_topics.output) {
    ph_output_ = pairs_lib::PublisherHandler<pairs_msgs::EstimatorOutput>(nh, getNamespacedName() + "/output", 10);
  }
  if (ch_->debug_topics.diag) {
    ph_diagnostics_ = pairs_lib::PublisherHandler<pairs_msgs::EstimatorDiagnostics>(nh, getNamespacedName() + "/diagnostics", 10);
  }
