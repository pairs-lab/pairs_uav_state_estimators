...
void LatGeneric::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  sh_hdg_state_ = pairs_lib::SubscriberHandler<pairs_msgs::msg::EstimatorOutput>(shopts, hdg_source_topic_);
...
bool LatGeneric::reset(void) {
  ...
  if (is_repredictor_enabled_) {
    ...
    lkf_rep_              = std::make_unique<pairs_lib::Repredictor<varstep_lkf_t>>(x0, P0, u0, Q_, t0, models_[0], rep_buffer_size_);
  }
