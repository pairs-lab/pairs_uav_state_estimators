...
void F9P::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  sh_f9p_odom_ = pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, msg_topic_, &F9P::callbackF9POdom, this);

  sh_gnss_ = pairs_lib::SubscriberHandler<sensor_msgs::msg::NavSatFix>(shopts, gnss_topic_, &F9P::callbackGnss, this);
...
void F9P::callbackGnss(const sensor_msgs::msg::NavSatFix::ConstSharedPtr msg) {
  ...
  pairs_lib::UTM(msg->latitude, msg->longitude, &gnss_x_, &gnss_y_);
...
/*//{ setUavState() */
bool F9P::setUavState([[maybe_unused]] const pairs_msgs::msg::UavState &uav_state) {
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_state_estimators::f9p::F9P, pairs_uav_managers::StateEstimator)
