...
void F9P::initialize(ros::NodeHandle &parent_nh, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  sh_f9p_odom_ = pairs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, msg_topic_, &F9P::callbackF9POdom, this);

  sh_gnss_ = pairs_lib::SubscribeHandler<sensor_msgs::NavSatFix>(shopts, gnss_topic_, &F9P::callbackGnss, this);
  ...
  ph_odom_ = pairs_lib::PublisherHandler<nav_msgs::Odometry>(nh, Support::toSnakeCase(getName()) + "/odom", 10);  // needed for tf
...
/*//{ setUavState() */
bool F9P::setUavState([[maybe_unused]] const pairs_msgs::UavState &uav_state) {
