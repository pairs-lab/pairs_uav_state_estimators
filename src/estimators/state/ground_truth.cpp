...
void GroundTruth::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  sh_gt_odom_ = pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, msg_topic_);

  // | ---------------- publishers initialization --------------- |

  ph_odom_ = pairs_lib::PublisherHandler<nav_msgs::msg::Odometry>(node_, "~/" + Support::toSnakeCase(getName()) + "/odom"); // needed for tf
...
void GroundTruth::timerCheckHealth() {
  ...
  if (isInState(INITIALIZED_STATE)) {
    ...
    if (sh_gt_odom_.hasMsg()) {
      ...
      error_publisher_->addWaitingForTopicError(sh_gt_odom_.topicName());
      return;
    }
  }
...
bool GroundTruth::setUavState([[maybe_unused]] const pairs_msgs::msg::UavState &uav_state) {
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_state_estimators::ground_truth::GroundTruth, pairs_uav_managers::StateEstimator)
