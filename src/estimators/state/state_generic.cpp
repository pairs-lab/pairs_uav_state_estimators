...
void StateGeneric::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  ph_attitude_ = pairs_lib::PublisherHandler<geometry_msgs::msg::QuaternionStamped>(node_, Support::toSnakeCase("~/" + getName()) + "/attitude");
...

  {
    ...
    est_ph->param_loader            = std::make_unique<pairs_lib::ParamLoader>(subnode);
...
void StateGeneric::timerUpdate() {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "StateGeneric::timerUpdate", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void StateGeneric::timerPubAttitude() {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer(node_, "StateGeneric::timerPubAttitude", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
/*//{ setUavState() */
bool StateGeneric::setUavState([[maybe_unused]] const pairs_msgs::msg::UavState &uav_state) {
