...
void StateGeneric::initialize(ros::NodeHandle &parent_nh, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
  ...
  ph_attitude_ = pairs_lib::PublisherHandler<geometry_msgs::QuaternionStamped>(nh, Support::toSnakeCase(getName()) + "/attitude", 10);
...
void StateGeneric::timerUpdate([[maybe_unused]] const ros::TimerEvent &event) {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("StateGeneric::timerUpdate", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void StateGeneric::timerCheckHealth([[maybe_unused]] const ros::TimerEvent &event) {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("StateGeneric::timerCheckHealth", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
void StateGeneric::timerPubAttitude([[maybe_unused]] const ros::TimerEvent &event) {
  ...
  pairs_lib::ScopeTimer scope_timer = pairs_lib::ScopeTimer("StateGeneric::timerPubAttitude", ch_->scope_timer.logger, ch_->scope_timer.enabled);
...
/*//{ setUavState() */
bool StateGeneric::setUavState([[maybe_unused]] const pairs_msgs::UavState &uav_state) {
