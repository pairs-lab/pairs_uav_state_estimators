...
int correctionTfTestImpl(const std::vector<double> data_vec) {
  ...
  std::shared_ptr<pairs_uav_managers::estimation_manager::CommonHandlers_t> ch = std::make_shared<pairs_uav_managers::estimation_manager::CommonHandlers_t>();
  ch->transformer                      = std::make_shared<pairs_lib::Transformer>(nh, name);
  ...
  ros::Publisher pub_rtk_msg = nh.advertise<pairs_msgs::RtkGps>("/rtk_test/rtk_msg", 1);
  ...
  pairs_uav_state_estimators::Correction<n_measurements> correction(nh, name, "lat_rtk/pos_rtk", "uav1/rtk_origin", pairs_uav_state_estimators::EstimatorType_t::LATERAL, ch);
...
TEST(TESTSuite, correctionTf) {
  ...
  const std::string    name = "corrections_test";
  ros::NodeHandle      nh;
  pairs_lib::ParamLoader param_loader(nh, name);
  ...
  for (int i = 0; i < test_data.size(); i++) {
    ...
    if (data.size() > 6) {
      int result = pairs_uav_state_estimation::correctionTfTestImpl(data);
      EXPECT_TRUE(result);
      data.clear();
    }
  }
